...
STATIC_INLINE void write_jmp_target(uae_u32* jmpaddr, uintptr a)
{
	...
	// - Convert jmpaddr to its RX equivalent for correct offset calculation
	uintptr jmpaddr_rx = (uintptr)jit_rw_to_rx((void*)jmpaddr);
	uintptr off = (a - jmpaddr_rx) >> 2;
#else
	...
	if((*(jmpaddr) & 0xfc000000) == 0x14000000) {
		...
		*(jmpaddr) = (*(jmpaddr) & 0xfc000000) | off;
		...
		/* TBZ/TBNZ/CBZ/CBNZ */
#ifdef __ANDROID__
		if((a > jmpaddr_rx && off > 0x1fff) || (a < jmpaddr_rx && (~off) > 0x1fff))
#else
		...
		*(jmpaddr) = (*(jmpaddr) & 0xfffc001f) | (off << 5);
		...
		/* conditional branch */
#ifdef __ANDROID__
		if((a > jmpaddr_rx && off > 0x3ffff) || (a < jmpaddr_rx && (~off) > 0x3ffff))
#else
