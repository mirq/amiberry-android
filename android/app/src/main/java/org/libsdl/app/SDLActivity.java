...
//        }
    ...
    // ============================================================
    // Amiberry Android File Picker Integration
    // Uses Storage Access Framework (SAF) for secure file/folder access
    // ============================================================
    
    private static final int REQUEST_CODE_OPEN_DOCUMENT_TREE = 42001;
    private static final int REQUEST_CODE_OPEN_DOCUMENT = 42002;
    
    // Synchronization for blocking folder picker
    private static final Object sFilePickerLock = new Object();
    private static String sSelectedPath = null;
    private static boolean sFilePickerCompleted = false;
    
    /**
     * Launch Android's native folder picker (called from JNI)
     * This method blocks until the user selects a folder or cancels
     */
    public static String showFolderPicker(String title) {
        if (mSingleton == null) {
            Log.e(TAG, "showFolderPicker: mSingleton is null");
            return "";
        }
        
        synchronized (sFilePickerLock) {
            sSelectedPath = null;
            sFilePickerCompleted = false;
        }
        
        // Launch folder picker on UI thread
        mSingleton.runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    mSingleton.startActivityForResult(intent, REQUEST_CODE_OPEN_DOCUMENT_TREE);
                    Log.v(TAG, "Folder picker launched");
                } catch (Exception e) {
                    Log.e(TAG, "Failed to launch folder picker: " + e.getMessage());
                    synchronized (sFilePickerLock) {
                        sFilePickerCompleted = true;
                        sFilePickerLock.notifyAll();
                    }
                }
            }
        });
        ...
        // Wait for result
        synchronized (sFilePickerLock) {
            while (!sFilePickerCompleted) {
                try {
                    sFilePickerLock.wait();
                } catch (InterruptedException e) {
                    Log.e(TAG, "Folder picker wait interrupted");
                    return "";
                }
            }
            return sSelectedPath != null ? sSelectedPath : "";
        }
    }
    
    /**
     * Launch Android's native file picker (called from JNI)
     * This method blocks until the user selects a file or cancels
     */
    public static String showFilePicker(String title, String mimeTypes) {
        if (mSingleton == null) {
            Log.e(TAG, "showFilePicker: mSingleton is null");
            return "";
        }
        ...
        mSingleton.runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                    mSingleton.startActivityForResult(intent, REQUEST_CODE_OPEN_DOCUMENT);
                    Log.v(TAG, "File picker launched");
                } catch (Exception e) {
        ...
        synchronized (sFilePickerLock) {
            while (!sFilePickerCompleted) {
                try {
                    ...
                    Log.e(TAG, "File picker wait interrupted");
                    return "";
                }
            }
            return sSelectedPath != null ? sSelectedPath : "";
    ...
    private void handleFolderPickerResult(int resultCode, Intent data) {
        String path = null;
        
        if (resultCode == RESULT_OK && data != null) {
            Uri treeUri = data.getData();
            if (treeUri != null) {
                Log.v(TAG, "Folder picker result URI: " + treeUri.toString());
                
                // Take persistable permission
                try {
                    final int takeFlags = Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION;
                    getContentResolver().takePersistableUriPermission(treeUri, takeFlags);
                    Log.v(TAG, "Persistable permission granted for: " + treeUri.toString());
                } catch (Exception e) {
                    Log.e(TAG, "Failed to take persistable permission: " + e.getMessage());
                }
                
                // Convert URI to filesystem path
                path = getPathFromTreeUri(treeUri);
                Log.v(TAG, "Folder picker resolved path: " + path);
            }
        } else {
            Log.v(TAG, "Folder picker cancelled or failed");
        }
    ...
    private void handleFilePickerResult(int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && data != null) {
            ...
            if (fileUri != null) {
                Log.v(TAG, "File picker result URI: " + fileUri.toString());
                ...
                try {
                    ...
                    Log.w(TAG, "Failed to take persistable permission (might not be persistable): " + e.getMessage());
                }
                
                // Convert URI to filesystem path
                path = getPathFromUri(fileUri);
                Log.v(TAG, "File picker resolved path: " + path);
            }
        } else {
            Log.v(TAG, "File picker cancelled or failed");
        }
    ...
    private String getPathFromTreeUri(Uri treeUri) {
        ...
        if (docId.startsWith("primary:")) {
            String relativePath = docId.substring(8); // Remove "primary:"
            ...
            for (java.io.File dir : externalDirs) {
                if (dir != null) {
                    ...
                    if (dirPath.contains(volumeId)) {
                        ...
                        if (androidIndex > 0) {
                            String storagePath = dirPath.substring(0, androidIndex);
                            if (relativePath.isEmpty()) {
                                return storagePath;
                            }
        ...
        // Last resort: try to use the URI directly with DocumentFile
        Log.w(TAG, "Could not resolve tree URI to filesystem path, returning URI string");
        return treeUri.toString();
    }
    
    /**
     * Convert a document (file) URI to a filesystem path
     */
    private String getPathFromUri(Uri uri) {
        ...
        if ("content".equalsIgnoreCase(scheme)) {
            ...
            try {
                ...
                Log.w(TAG, "Not a document URI: " + e.getMessage());
            }
            
            if (docId != null) {
                Log.v(TAG, "Document ID: " + docId);
                
                // Handle primary storage
                if (docId.startsWith("primary:")) {
                    String relativePath = docId.substring(8);
                    return android.os.Environment.getExternalStorageDirectory().getAbsolutePath() + "/" + relativePath;
                }
                
                // Handle raw file paths (some file managers return these)
                    ...
                    for (java.io.File dir : externalDirs) {
                        if (dir != null) {
                            ...
                            if (dirPath.contains(volumeId)) {
                                int androidIndex = dirPath.indexOf("/Android/");
                                if (androidIndex > 0) {
                                    return dirPath.substring(0, androidIndex) + "/" + relativePath;
                                }
                            }
                        }
                    }
            ...
            // If we couldn't resolve to a path, return the URI string
            // The C++ code can handle content:// URIs via Android APIs if needed
            Log.w(TAG, "Could not resolve content URI to filesystem path");
            return uri.toString();
        }
    ...
    /**
     * Get the app's internal storage path (always accessible)
     */
    public static String getInternalStoragePath() {
        if (mSingleton != null) {
            return mSingleton.getFilesDir().getAbsolutePath();
        }
        return "";
    }
